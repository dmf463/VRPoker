...
inline float3 UnityWorldToObjectPos( in float3 pos )
{
	return mul(matWorldToObject, float4(pos, 1.0)).xyz;
}
...
half4 fragShared(v2f i, float outsideBeam)
{
    ...
    // View dir vector
    float3 viewDirObjectSpaceN = normalize(i.posObjectSpace.xyz - camPosObjectSpace);
    ...
    // range factor
    float distFromSourceNormalized = saturate(smoothstep(_DistanceFadeStart, _DistanceFadeEnd, pixDistFromSource));
    float rangeFactor = saturate(1 - distFromSourceNormalized);

    // Noise factor
    float noise3DFactor = GetNoise3DFactor(i.posWorldSpace);
    noise3DFactor = lerpClamped(noise3DFactor, 1, rangeFactor * 0.1);
    ...
    float viewDirDotPlusZ = dot(viewDirObjectSpaceN, float3(0, 0, 1));
    ...
    {
        // real fresnel factor
        float fresnelReal = dot(normalObjectSpace, -viewDirObjectSpaceN);
        ...
        float distToPlane = dot(-viewDirObjectSpaceN, tangentPlaneNormal);
        float3 vec2D = normalize(-viewDirObjectSpaceN - distToPlane * tangentPlaneNormal);
